...
def get_pygame_screen_font(h: int, size: int, flags: int = 0):
    ...
    *3h/4*, *5h/6*.
    ...
    try:
        ...
        raise RuntimeError(f"Unable to create a screen, flags={flags}") from e
    font = pygame.font.Font("freesansbold.ttf", h)
    font_small = pygame.font.Font("freesansbold.ttf", 3 * h // 4)
        ...
        if progress is None:
            ...
            for opcode in diff.get_opcodes():
                if opcode[0] in {"delete", "equal", "insert"}:
                    ...
                    opcodes.append(("delete", opcode[1], opcode[2], None, None))
                    opcodes.append(("insert", None, None, opcode[3], opcode[4]))
                else:
                ...
                if opcode[0] == "delete":
                    for i in range(opcode[1], opcode[2]):
                        row = (
                            seq1[i],
                            i,
                            seq2.index(seq1[i]) if seq1[i] in seq2 else None,
                    ...
                    for i in range(opcode[3], opcode[4]):
                        row = (
                            seq2[i],
                            seq1.index(seq2[i]) if seq2[i] in seq1 else None,
                            i,
                            bars[i] if bars is not None else None,
                        )
                        positions.append(row)
            for text, p1, p2, bar_ in positions:
                ...
                if bar_ is not None:
                    ...
                    pygame.draw.line(
                        screen, color_bar, (x, y2), (x + int(bar_ * maxw), y2), width
                    )
                text = font.render(text, True, color)
